...
class LinearShortestPath(Kernel):
    def __init__(self, n_jobs=None,
                 normalize=False,
                 verbose=False,
                 with_labels=True,
                 algorithm_type="auto"):
        """Initialize a `linear_shortest_path` kernel."""
        super(LinearShortestPath, self).__init__(
            n_jobs=n_jobs, normalize=normalize, verbose=verbose)
    ...
    def initialize(self):
        """Initialize all transformer arguments, needing initialization."""
        super(LinearShortestPath, self).initialize()
        if not self._initialized["algorithm_type"]:
            if self.algorithm_type == "auto":
                self._graph_format = "auto"
            elif self.algorithm_type == "floyd_warshall":
    ...
    def pairwise_operation(self, x, y):
        """Calculate linear shortests paths.
            ...
            for j in range(dimx):
                ...
                for k in range(dimy):
                    for m in range(dimy):
                        if k == m:
                            continue
                        if Sx[i, j] != float('Inf') and Sy[k, m] != float('Inf'):
                            if phi_x[i] == phi_y[k]:
                                k_ik = 1
                            if phi_x[j] == phi_y[m]:
                                k_jm = 1
                            ...
                            if phi_x[j] == phi_y[k]:
                                k_jk = 1
                            k_l = Sx[i, j] * Sy[k, m]
                            kernel += (k_ik * k_jm * k_l) + (k_im * k_jk * k_l)
        return kernel
