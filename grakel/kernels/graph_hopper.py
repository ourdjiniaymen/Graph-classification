    ...
    def parse_input(self, X):
        ...
        for i in range(ni):
            ...
            if self.calculate_norm_:
                out.append((M, attributes, custom_sum(attributes ** 2, axis=1)))
            else:
...
def linear_kernel(x, y):
    ...
    NA_linear_kernel = custom_dot(NA_i, NA_j.T)
    return np.dot(np.sum(weight_matrix.flat), NA_linear_kernel.flat)
...
def gaussian_kernel(x, y, mu):
    ...
    return np.dot(np.sum(weight_matrix.flat), nodepair.flat)
...
def bridge_kernel(x, y):
    ...
    a = np.dot(np.sum(weight_matrix.flat), nodepair.flat)
    if not a:
        return np.array([0])
    return a
...
def custom_dot(x, y):
    ...
    x_copy = x.flatten()
    y_copy = y.flatten()
    diff = abs(len(x_copy) - len(y_copy))
    x_copy, y_copy = arrange_array(x_copy, y_copy)
    return np.dot(x_copy, np.concatenate((y_copy, [0] * diff)))
...
def custom_vstack(l):
    ...
    diff = abs(len(x_copy) - len(y_copy))
    x_copy, y_copy = arrange_array(x_copy, y_copy)
    return np.vstack([x_copy, np.concatenate((y_copy, [0] * diff))])


def arrange_array(x, y):
    if len(x) > len(y):
        return x, y
    return y, x


def custom_sum(x, axis):
    if x.ndim == 1:
        return np.sum(x)
